 ...
 * the lightweight SPV mode, a {@link com.google.marscoin.store.BoundedOverheadBlockStore} may be a good choice.</p>
 *
 ...
 * {@link com.google.marscoin.store.H2FullPrunedBlockStore} implements full verification, which is equivalent to the
...
public abstract class AbstractBlockChain {
     ...
     *
     * Following this one down to the genesis block produces the story of the economy from the creation of marscoin
    ...
    private boolean add(Block block, Set<Sha256Hash> filteredTxHashList, List<Transaction> filteredTxn, boolean tryConnecting)
        ...
        try {
            ...
            // article here for more details: http://code.google.com/p/marscoinj/wiki/SecurityModel
            try {
                block.verifyHeader();
        ...
        for (Transaction tx : transactions) {
            try {
                if (listener.isTransactionRelevant(tx)) {
                    if (clone)
                        tx = new Transaction(tx.params, tx.marscoinSerialize());
	...
	if(cursor == null) return;
        ...
        if (timespan < Ttimespan / 4)
            timespan = Ttimespan / 4;
        if (timespan > Ttimespan * 4)
            timespan = Ttimespan * 4;

        BigInteger newDifficulty = Utils.decodeCompactBits(prev.getDifficultyTarget());
        newDifficulty = newDifficulty.multiply(BigInteger.valueOf(timespan));
        newDifficulty = newDifficulty.divide(BigInteger.valueOf(Ttimespan));
    ...
    private void checkTestnetDifficulty(StoredBlock storedPrev, Block prev, Block next) throws VerificationException, BlockStoreException {
        ...
        // There is an integer underflow bug in marscoin-qt that means mindiff blocks are accepted when time
        // goes backwards.
