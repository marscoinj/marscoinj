...
/**
 * <p>A Message is a data structure that can be serialized/deserialized using both the marscoin proprietary serialization
...
public abstract class Message implements Serializable {
     ...
     * @param msg marscoin protocol formatted byte array containing message content.
     * @param offset The location of the first msg byte within the array.
     * @param protocolVersion marscoin protocol version.
    ...
    private void selfCheck(byte[] msg, int offset) {
        if (!(this instanceof VersionMessage)) {
            ...
            byte[] reserialized = marscoinSerialize();
    ...
    // These methods handle the serialization/deserialization using the custom marscoin protocol.
    ...
    /**
     * Should only used by marscoinSerializer for cached checksum
     *
     * @return the checksum
     */
    byte[] getChecksum() {
    ...
    /**
     * Should only used by marscoinSerializer for caching checksum
     *
    ...
    /**
     * Returns a copy of the array returned by {@link Message#unsafemarscoinSerialize()}, which is safe to mutate.
     ...
     */
    public byte[] marscoinSerialize() {
        byte[] bytes = unsafemarscoinSerialize();
    ...
    /**
     * Serialize this message to a byte array that conforms to the marscoin wire protocol.
     * <br/>
     ...
     */
    public byte[] unsafemarscoinSerialize() {
        ...
        try {
            marscoinSerializeToStream(stream);
        } catch (IOException e) {
        ...
        if (parseRetain) {
            ...
            // which means another call to marscoinSerialize is coming.  If we didn't recache then internal
    ...
    /**
     * Serialize this message to the provided OutputStream using the marscoin wire format.
     *
     * @param stream
     * @throws IOException
     */
    final public void marscoinSerialize(OutputStream stream) throws IOException {
    ...
    /**
     * Serializes this message to the provided stream. If you just want the raw bytes use marscoinSerialize().
     */
    void marscoinSerializeToStream(OutputStream stream) throws IOException {
        log.debug("Warning: {} class has not implemented marscoinSerializeToStream method.  Generating message with no payload", getClass());
    }

    /**
     * This method is a NOP for all classes except Block and Transaction.  It is only declared in Message
     * so marscoinSerializer can avoid 2 instanceof checks + a casting.
     *
     * @return
     */
