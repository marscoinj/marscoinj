...
import static com.google.marscoin.core.Utils.*;

/**
 * <p>A transaction represents the movement of coins from some addresses to some other addresses. It can also represent
 * the minting of new coins. A Transaction object corresponds to the equivalent in the marscoin C++ implementation.</p>
 *
 * <p>Transactions are the fundamental atoms of marscoin and have many powerful features. Read
 * <a href="http://code.google.com/p/marscoinj/wiki/WorkingWithTransactions">"Working with transactions"</a> in the
 * documentation to learn more about how to use this class.</p>
 *
 * <p>All marscoin transactions are at risk of being reversed, though the risk is much less than with traditional payment
...
public class Transaction extends ChildMessage implements Serializable {
    ...
    // These are serialized in both marscoin and java serialization.
    ...
    /**
     * Used by marscoinSerializer.  The serializer has to calculate a hash for checksumming so to
     ...
     * value is determined by a formula that all implementations of marscoin share. In 2011 the value of a coinbase
    ...
    public String toString(AbstractBlockChain chain) {
        ...
        for (TransactionOutput out : outputs) {
            ...
            try {
                ...
                s.append(" ");
                s.append(marscoinValueToFriendlyString(out.getValue()));
                s.append(" BTC");
     ...
     * features of marscoin are developed.
     *
    ...
    synchronized Sha256Hash hashTransactionForSignature(int inputIndex, byte[] connectedScript,
        ...
        //
        //   https://en.marscoin.it/wiki/Contracts
        ...
        try {
            ...
            // is a legacy holdover from a previous, broken design of executing scripts that shipped in marscoin 0.1.
            ...
            if ((sigHashType & 0x1f) == (SigHash.NONE.ordinal() + 1)) {
                ...
                if (inputIndex >= this.outputs.size()) {
                    ...
                    // marscoin implementation. The reference client also contains a bug in handling this case:
    ...
    @Override
    protected void marscoinSerializeToStream(OutputStream stream) throws IOException {
        uint32ToByteStreamLE(version, stream);
        stream.write(new VarInt(inputs.size()).encode());
        for (TransactionInput in : inputs)
            in.marscoinSerialize(stream);
        stream.write(new VarInt(outputs.size()).encode());
        for (TransactionOutput out : outputs)
            out.marscoinSerialize(stream);
     ...
     * This is useful in certain types of <a href="http://en.marscoin.it/wiki/Contracts">contracts</a>, such as
