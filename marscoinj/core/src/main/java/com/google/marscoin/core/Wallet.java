...
import com.google.marscoin.crypto.KeyCrypterScrypt;
import org.marscoinj.wallet.Protos.Wallet.EncryptionType;
import org.spongycastle.crypto.params.KeyParameter;

import com.google.marscoin.core.TransactionConfidence.ConfidenceType;
import com.google.marscoin.core.WalletTransaction.Pool;
import com.google.marscoin.crypto.KeyCrypter;
import com.google.marscoin.crypto.KeyCrypterException;
import com.google.marscoin.store.WalletProtobufSerializer;
import com.google.marscoin.utils.Locks;
...
import static com.google.marscoin.core.Utils.marscoinValueToFriendlyString;
 ...
 * of the marscoin protocol.</p>
 *
 * <p>To learn more about this class, read <b><a href="http://code.google.com/p/marscoinj/wiki/WorkingWithTheWallet">
 ...
 * other objects, see the <a href="http://code.google.com/p/marscoinj/wiki/GettingStarted">Getting started</a> tutorial
 * on the website to learn more about how to set everything up.</p>
 *
 * <p>Wallets can be serialized using either Java serialization - this is not compatible across versions of marscoinj,
 ...
 * {@link Wallet#autosaveToFile(java.io.File, long, java.util.concurrent.TimeUnit, com.google.marscoin.core.Wallet.AutosaveEventListener)}
...
public class Wallet implements Serializable, BlockChainListener {
     ...
     * that handles this in marscoin C++ is complicated. Satoshis code will not allow you to spend unconfirmed coins,
        ...
        try {
            ...
            if (log.isInfoEnabled()) {
                log.info(String.format("Received a pending transaction %s that spends %s BTC from our own wallet," +
                        " and sends us %s BTC", tx.getHashAsString(), Utils.marscoinValueToFriendlyString(valueSentFromMe),
                        Utils.marscoinValueToFriendlyString(valueSentToMe)));
            }
    ...
    private void receive(Transaction tx, StoredBlock block, BlockChain.NewBlockType blockType, boolean reorg) throws VerificationException {
        ...
        if (!reorg) {
            log.info("Received tx {} for {} BTC: {}", new Object[]{sideChain ? "on a side chain" : "",
                    marscoinValueToFriendlyString(valueDifference), tx.getHashAsString()});
        }
	...
	// Implements revision d64f55589694
        ...
        log.info("Balance is now: " + marscoinValueToFriendlyString(getBalance()));
    ...
    public static class SendResult {
        /** The marscoin transaction message that moves the money. */
    ...
    public static class SendRequest {
         ...
         * is how mining is incentivized in later years of the marscoin system when inflation drops. It also provides
         * a way for people to prioritize their transactions over others and is used as a way to make denial of service
         * attacks expensive. Some transactions require a fee due to their structure - currently marscoinj does not
     ...
     * followed by {@link Wallet#completeTx(com.google.marscoin.core.Wallet.SendRequest)} and returning the requests
     ...
     *
     * @param address       The marscoin address to send the money to.
     ...
     * {@link Wallet#completeTx(com.google.marscoin.core.Wallet.SendRequest)} to make it valid.
     *
    ...
    public boolean completeTx(SendRequest req) {
        ...
        try {
            ...
            log.info("Completing send tx with {} outputs totalling {}",
                    req.tx.getOutputs().size(), marscoinValueToFriendlyString(value));
            ...
            if (selection.valueGathered.compareTo(value) < 0) {
                log.warn("Insufficient value in wallet for send, missing " +
                        marscoinValueToFriendlyString(value.subtract(selection.valueGathered)));
            ...
            if (change.compareTo(BigInteger.ZERO) > 0) {
                ...
                log.info("  with {} coins change", marscoinValueToFriendlyString(change));
            ...
            // Check size.
            int size = req.tx.marscoinSerialize().length;
     ...
     * If {@link Wallet#autosaveToFile(java.io.File, long, java.util.concurrent.TimeUnit, com.google.marscoin.core.Wallet.AutosaveEventListener)}
    ...
    public String toString(boolean includePrivateKeys, AbstractBlockChain chain) {
        ...
        try {
            ...
            builder.append(String.format("Wallet containing %s BTC in:%n", marscoinValueToFriendlyString(getBalance())));
    ...
    private void toStringHelper(StringBuilder builder, Map<Sha256Hash, Transaction> transactionMap,
        ...
        for (Transaction tx : transactionMap.values()) {
            try {
                ...
                builder.append(Utils.marscoinValueToFriendlyString(tx.getValueSentFromMe(this)));
                builder.append(" and receives ");
                builder.append(Utils.marscoinValueToFriendlyString(tx.getValueSentToMe(this)));
                builder.append(", total value ");
                builder.append(Utils.marscoinValueToFriendlyString(tx.getValue(this)));
                builder.append(".\n");
            } catch (ScriptException e) {
    ...
    public void reorganize(StoredBlock splitPoint, List<StoredBlock> oldBlocks, List<StoredBlock> newBlocks) throws VerificationException {
        ...
        try {
            ...
            log.info("post-reorg balance is {}", Utils.marscoinValueToFriendlyString(getBalance()));
     ...
     * {@link com.google.marscoin.core.ECKey#getCreationTimeSeconds()}. This can return zero if at least one key does
    ...
    /**
     * Convenience wrapper around {@link Wallet#encrypt(com.google.marscoin.crypto.KeyCrypter,
     ...
     * {@link com.google.marscoin.crypto.KeyCrypterScrypt}.
     *
        ...
        try {
            ...
            for (ECKey key : keychain) {
                if (key.isEncrypted()) {
                    ...
                    // (all marscoin controlled by that private key is lost forever).
    ...
    /**
     * <p>Convenience wrapper around {@link Wallet#addNewEncryptedKey(com.google.marscoin.crypto.KeyCrypter,
            ...
            for (int i = 0; i < tx.getOutputs().size(); i++) {
                ...
                try {
                    if (out.isMine(this) && out.getScriptPubKey().isSentToRawPubKey()) {
                        ...
                        filter.insert(outPoint.marscoinSerialize());
                    }
                } catch (ScriptException e) {
                    throw new RuntimeException(e); // If it is ours, we parsed the script corectly, so this shouldn't happen
                }
