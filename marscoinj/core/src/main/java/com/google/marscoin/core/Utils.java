...
/**
 * A collection of various utility methods that are helpful for working with the marscoin protocol.
 * To enable debug logging from the library, run with -Dmarscoinj.logging=true on your command line.
 */
public class Utils {
    ...
    /** The string that prefixes all text messages signed using marscoin keys. */
    public static final String MARSCOIN_SIGNED_MESSAGE_HEADER = "Marscoin Signed Message:\n";

    // TODO: Replace this nanocoins business with something better.

    /**
     * How many "nanocoins" there are in a marscoin.
     * <p/>
     * A nanocoin is the smallest unit that can be transferred using marscoin.
    ...
    /**
     * How many "nanocoins" there are in 0.01 marscoins.
     * <p/>
     * A nanocoin is the smallest unit that can be transferred using marscoin.
     * The term nanocoin is very misleading, though, because there are only 100 million
     * of them in a coin (whereas one would expect 1 billion).
     ...
     * standard procedure in marscoin. The resulting hash is in big endian form.
     */
     ...
     */
    public static String marscoinValueToFriendlyString(BigInteger value) {
     ...
     */
    public static String marscoinValueToPlainString(BigInteger value) {
        if (value == null) {
     ...
     *
     * <tt><p>[24] "Marscoin Signed Message:\n" [message.length as a varint] message</p></tt>
     */
    public static byte[] formatMessageForSigning(String message) {
        VarInt size = new VarInt(message.length());
        int totalSize = 1 + MARSCOIN_SIGNED_MESSAGE_HEADER.length() + size.getSizeInBytes() + message.length();
        byte[] result = new byte[totalSize];
        int cursor = 0;
        result[cursor++] = (byte) MARSCOIN_SIGNED_MESSAGE_HEADER.length();
        byte[] bytes = MARSCOIN_SIGNED_MESSAGE_HEADER.getBytes(Charset.forName("UTF-8"));
