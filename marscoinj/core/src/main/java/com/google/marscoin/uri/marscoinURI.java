...
import com.google.marscoin.core.Address;
import com.google.marscoin.core.AddressFormatException;
import com.google.marscoin.core.NetworkParameters;
import com.google.marscoin.core.Utils;
...
/**
 * <p>Provides a standard implementation of a marscoin URI with support for the
 * following:</p>
 *
 * <ul>
 ...
 *
 * <ul>
 * <li>{@code marscoin:<address>}</li>
 * <li>{@code marscoin:<address>?<name1>=<value1>&<name2>=<value2>} with multiple
 ...
 * @see <a href="https://en.marscoin.it/wiki/BIP_0021">BIP 0021</a>
 */
public class marscoinURI {
    /**
     * Provides logging for this class
     */
    private static final Logger log = LoggerFactory.getLogger(marscoinURI.class);
    ...
    public static final String MARSCOIN_SCHEME = "marscoin";
    ...
    /**
     * Constructs a new marscoinURI from the given string. Can be for any network.
     *
     * @param uri The raw URI data to be parsed (see class comments for accepted formats)
     * @throws marscoinURIParseException if the URI is not syntactically or semantically valid.
     */
    public marscoinURI(String uri) {
        this(null, uri);
    }
     ...
     * @throws marscoinURIParseException If the input fails marscoin URI syntax and semantic checks.
     */
    public marscoinURI(NetworkParameters params, String input) {
        // Basic validation
        ...
        try {
            ...
            throw new marscoinURIParseException("Bad URI syntax", e);
        }

        // URI is formed as  marscoin:<address>?<query parameters>
        // blockchain.info generates URIs of non-BIP compliant form marscoin://address?....
        // We support both until Ben fixes his code.
        
        // Remove the marscoin scheme.
        // (Note: getSchemeSpecificPart() is not used as it unescapes the label and parse then fails.
        // For instance with : marscoin:129mVqKUmJ9uwPxKJBnNdABbuaaNfho4Ha?amount=0.06&label=Tom%20%26%20Jerry
        // the & (%26) in Tom and Jerry gets interpreted as a separator and the label then gets parsed
        // as 'Tom ' instead of 'Tom & Jerry')
        String schemeSpecificPart;
        if (input.startsWith("marscoin://")) {
            schemeSpecificPart = input.substring("marscoin://".length());
        } else if (input.startsWith("marscoin:")) {
            schemeSpecificPart = input.substring("marscoin:".length());
        } else {
            throw new marscoinURIParseException("Unsupported URI scheme: " + uri.getScheme());
        }

        // Split off the address from the rest of the query parameters.
        String[] addressSplitTokens = schemeSpecificPart.split("\\?");
        if (addressSplitTokens.length == 0 || "".equals(addressSplitTokens[0])) {
            throw new marscoinURIParseException("Missing address");
        }
        ...
        if (addressSplitTokens.length == 1) {
            ...
            if (addressSplitTokens.length == 2) {
                ...
                throw new marscoinURIParseException("Too many question marks in URI '" + uri + "'");
            }
        }
    ...
    private void parseParameters(NetworkParameters params, String addressToken, String[] nameValuePairTokens) {
        // Attempt to parse the addressToken as a marscoin address for this network
        try {
            Address address = new Address(params, addressToken);
            putWithValidation(FIELD_ADDRESS, address);
        } catch (final AddressFormatException e) {
            throw new marscoinURIParseException("Bad address", e);
        }
        ...
        for (String nameValuePairToken : nameValuePairTokens) {
            ...
            if (tokens.length != 2 || "".equals(tokens[0])) {
                throw new marscoinURIParseException("Malformed marscoin URI - cannot parse name value pair '" +
                        nameValuePairToken + "'");
            }
    ...
    private void putWithValidation(String key, Object value) {
        if (parameterMap.containsKey(key)) {
            throw new marscoinURIParseException("'" + key + "' is duplicated, URI is invalid");
        } else {
    ...
    /**
     * @return The marscoin Address from the URI
     */
    ...
    public String toString() {
        StringBuilder builder = new StringBuilder("marscoinURI[");
        boolean first = true;
    ...
    public static String convertTomarscoinURI(Address address, BigInteger amount, String label, String message) {
        return convertTomarscoinURI(address.toString(), amount, label, message);
    }

    /**
     * Simple marscoin URI builder using known good fields.
     * 
     * @param address The marscoin address
     * @param amount The amount in nanocoins (decimal)
     * @param label A label
     * @param message A message
     * @return A String containing the marscoin URI
     */
    public static String convertTomarscoinURI(String address, BigInteger amount, String label, String message) {
        ...
        builder.append(MARSCOIN_SCHEME).append(":").append(address);
        
        boolean questionMarkHasBeenOutput = false;
        
        if (amount != null) {
            builder.append(QUESTION_MARK_SEPARATOR).append(FIELD_AMOUNT).append("=");
            builder.append(Utils.marscoinValueToPlainString(amount));
