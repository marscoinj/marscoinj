...
package de.schildbach.wallet.marscoin.integration.android;
 ...
 */
public final class marscoinIntegration
{
	private static final String INTENT_EXTRA_TRANSACTION_HASH = "transaction_hash";

	/**
	 * Request any amount of marscoins (probably a donation) from user, without feedback from the app.
	 * 
	 * @param context
	 *            Android context
	 * @param address
	...
	/**
	 * Request specific amount of marscoins from user, without feedback from the app.
	 * 
	 * @param context
	 *            Android context
	 * @param address
	...
	/**
	 * Request any amount of marscoins (probably a donation) from user, with feedback from the app. Result intent can be
	 ...
	 * 
	 * Warning: A success indication is no guarantee! To be on the safe side, you must drive your own marscoin
	...
	/**
	 * Request specific amount of marscoins from user, with feedback from the app. Result intent can be received by
	...
	/**
	 * Put transaction hash into result intent. Meant for usage by marscoin wallet applications.
	 * 
	 * @param result
	 *            result intent
	 * @param txHash
	...
	/**
	 * Get transaction hash from result intent. Meant for usage by applications initiating a marscoin payment.
	 * 
	 * You can use this hash to request the transaction from the marscoin network, in order to validate. For this, you
	 * need your own marscoin infrastructure though. There is no guarantee that the transaction has ever been broadcasted
	 * to the marscoin network.
	 * 
	 * @param result
	 *            result intent
	...
	{
		final StringBuilder uri = new StringBuilder("marscoin:");
		if (address != null)
			uri.append(address);
		if (amount != null)
	...
	{
		Toast.makeText(context, "No marscoin application found.\nPlease install marscoin wallet.", Toast.LENGTH_LONG).show();

		final Intent marketIntent = new Intent(Intent.ACTION_VIEW, Uri.parse("market://details?id=de.marscoin.wallet"));
		final Intent binaryIntent = new Intent(Intent.ACTION_VIEW, Uri.parse("http://github.com/marscoin"));
