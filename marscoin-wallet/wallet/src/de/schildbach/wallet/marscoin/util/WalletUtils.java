...
package de.schildbach.wallet.marscoin.util;
...
import com.google.marscoin.core.DumpedPrivateKey;
import com.google.marscoin.core.ECKey;
import com.google.marscoin.core.ScriptException;
import com.google.marscoin.core.Sha256Hash;
import com.google.marscoin.core.Transaction;
import com.google.marscoin.core.TransactionInput;
import com.google.marscoin.core.TransactionOutput;
import com.google.marscoin.core.Utils;
import com.google.marscoin.uri.marscoinURI;
import com.google.marscoin.uri.marscoinURIParseException;
...
public class WalletUtils
{
	...
	public static void writeKeys(final Writer out, final List<ECKey> keys) throws IOException
	{
		...
		out.write("# KEEP YOUR PRIVATE KEYS SAFE! Anyone who can read this can spend your marscoins.\n");
    ...
    public static marscoinURI parseAddressString(String addressString) {
        if (addressString == null) return null;
        Log.d("marscoin", "Parsing: " + addressString);
        try {
            final marscoinURI uri = new marscoinURI(Constants.NETWORK_PARAMETERS, addressString);
            Log.d("marscoin", "URI: " + uri.getAddress().toString() + " " + uri.getLabel() + " " + uri.getAmount());
            return uri;
        }
        catch (final marscoinURIParseException x)
        {
            // Try prepending marscoin:
            try {
                final marscoinURI uri = new marscoinURI(Constants.NETWORK_PARAMETERS, "marscoin:" + addressString);
                Log.d("marscoin", "URI: " + uri.getAddress().toString() + " " + uri.getLabel() + " " + uri.getAmount());
                return uri;
            }
            catch (final marscoinURIParseException y)
            {
                return null;
            }
        }
